...
import { listReaderContent, mutateReaderContent, uploadReaderAsset, type ReaderContent, type ReaderKind, type ReaderMetadata } from '../../../lib/admin/firebase/reader-api';
...
export function ReaderContentManager({ kind }: { kind: ReaderKind }) {
  ...
  const fileControl = (slot: string, title: string, accept: string, hint: string) => (
    <label className={`block rounded-xl border border-dashed border-slate-700 p-4 ${busy || dirty || !selected ? 'opacity-40' : 'hover:border-emerald-500'}`}>
      ...
      <input aria-label={title} type="file" accept={accept} disabled={busy || dirty || !selected} className="mt-3 block w-full text-xs text-slate-300 file:mr-3 file:rounded-lg file:border-0 file:bg-slate-800 file:px-3 file:py-2 file:text-slate-200" onChange={event => { upload(slot, event.target.files?.[0]); event.target.value = ''; }} />
    </label>
  );
    ...
    <div className="grid items-start gap-6 xl:grid-cols-[minmax(260px,1fr)_minmax(0,2fr)]">
      <AdminCard className="p-5">
        ...
        <ul className="space-y-2">{items.map(item => <li key={item.id}><button disabled={busy} onClick={() => select(item)} className={`w-full rounded-xl border p-4 text-left ${selected?.id === item.id ? 'border-emerald-500 bg-emerald-950/30' : 'border-slate-800 hover:bg-slate-800/60'}`}>
          <span className="block truncate font-medium text-white">{item.title}</span><span className="mt-1 block text-xs text-slate-400">{item.author} · {item.published ? '공개 중' : '비공개'}</span>
      ...
      <AdminCard className="p-6">
        ...
        {selected && <div className="mt-8 flex flex-wrap items-center gap-3 border-t border-slate-800 pt-6">
          ...
          {selected.published && <button disabled={busy || dirty} className={buttonClass} onClick={() => void run(() => mutateReaderContent({ action: 'unpublish', id: selected.id, revision: selected.revision }), '비공개로 전환했습니다. 이미 내려받은 파일은 사용자 기기에 유지됩니다.')}>비공개로 전환</button>}
